...
{
public:
    Node *solve(int preorder[], int n, int mi, int ma, int &i)
    {
        if (i >= n)
        {
            return NULL;
        ...
        root->left = solve(preorder, n, mi, root->data, i);
        root->right = solve(preorder, n, root->data, ma, i);
        return root;
    }
